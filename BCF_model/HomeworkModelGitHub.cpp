               ...
               int keep_every)
{
  ...
  for(int iter = 0; iter < n_iter; iter++)
  {
    ...
    ////
    arma::mat y_resid = y_scaled - iter_preds - Z1%iter_preds_tau1- Z2%iter_preds_tau2- Z3%iter_preds_tau3- Z4%iter_preds_tau4;
    ...
    sigma=sample_sigma(n, v_0, y_scaled, iter_preds+Z1%iter_preds_tau1+Z2%iter_preds_tau2+Z3%iter_preds_tau3+Z4%iter_preds_tau4, sigma_0);
    
    if(iter>=n_burn & iter%keep_every==0)
    {
      arma::mat sigma_scaled = sample_sigma(n, v_0, y, preds_mat.slice(num_kept)+Z1%preds_mat_tau1.slice(num_kept)+Z2%preds_mat_tau2.slice(num_kept)+Z3%preds_mat_tau3.slice(num_kept)+Z4%preds_mat_tau4.slice(num_kept), sigma_0);
